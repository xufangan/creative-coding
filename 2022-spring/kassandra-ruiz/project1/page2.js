var myRec = new p5.SpeechRec('en-US'); // new P5.SpeechRec object
		...
		function draw() {
            ...
            let tempResult;
            tempResult = myRec.resultString.includes("absolutely")
            console.log(tempResult)
            if (tempResult == true){
                image(thumbs, 900, 200);
				text('excellent', 1000, 100, width, height)
            }


            tempResult = myRec.resultString.includes("not at all")
            console.log(tempResult)
            if (tempResult == true){
                image(jesus, 900, 200);
				text('thats too bad', 1000, 100, width, height)
            }
