var myRec = new p5.SpeechRec('en-US'); // new P5.SpeechRec object
        ...
        function preload() {
            jesus = loadImage('Pictures/pray.gif');
		...
		function draw() {
            ...
            let tempResult;
            tempResult = myRec.resultString.includes("cause")
            console.log(tempResult)
            if (tempResult == true){
                image(thumbs, 900, 200);
				text('You are right! It is completely subjective', 1000, 100, width, height)
            }


            tempResult = myRec.resultString.includes("breathe")
            console.log(tempResult)
            if (tempResult == true){
                image(thumbs, 900, 200);
				text('That is understandable', 1000, 100, width, height)
            }

			tempResult = myRec.resultString.includes("back")
            console.log(tempResult)
            if (tempResult == true){
                image(jesus, 900, 200);
				text('You are my role model', 1000, 100, width, height)
            }
