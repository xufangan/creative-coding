var myRec = new p5.SpeechRec('en-US'); // new P5.SpeechRec object
        ...
        function preload() {
            vampire = loadImage('Pictures/vampire.gif');
		...
		function draw() {
            ...
            let tempResult;
            tempResult = myRec.resultString.includes("nothing")
            console.log(tempResult)
            if (tempResult == true){
                image(vampire, 900, 200);
				text('Are you a vampire?', 1000, 100, width, height)
            }


            tempResult = myRec.resultString.includes("less")
            console.log(tempResult)
            if (tempResult == true){
                image(thumbs, 900, 200);
				text('No problem! I have not either', 1000, 100, width, height)
            }

			tempResult = myRec.resultString.includes("jealous")
            console.log(tempResult)
            if (tempResult == true){
                image(thumbs, 900, 200);
				text('Okay, yes, I am green with envy', 1000, 100, width, height)
            }
